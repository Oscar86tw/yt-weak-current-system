...
document.getElementById('app').innerHTML = layout('報價查詢', `
<div class="list-panel">
  <div class="toolbar"><input id="quoteSearch" placeholder="輸入客戶 / 工程 / 報價單號查詢"></div>
  <div id="quoteList"></div>
</div>`);
let rows = [];
async function loadQuotes(){ rows = await API.listQuotes(); render(); }
function render(){
  const k = quoteSearch.value.trim().toLowerCase();
  const list = rows.filter(q => `${q.quote_no||''} ${q.client_name||''} ${q.project_name||''}`.toLowerCase().includes(k) || !k);
...
loadQuotes().catch(err => alert(err.message));
